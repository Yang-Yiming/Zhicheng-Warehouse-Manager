...
declare
  ...
  now_shanghai_text text := to_char(timezone('asia/shanghai', now()), 'YYYY-MM-DD HH24:MI');
begin
  ...
  delete from public.operations where true;
  ...
  if p_mode = 'full' then
    ...
    loop
      ...
      insert into public.operations (
        ...
        coalesce(nullif(op->>'submitTime', '')::timestamptz, now_utc),
    ...
    loop
      insert into public.inventory (
        ...
        coalesce(nullif(btrim(coalesce(inv->>'lastOperationTime', '')), ''), now_shanghai_text),
        coalesce(inv->>'notes', ''),
        now_utc
      );

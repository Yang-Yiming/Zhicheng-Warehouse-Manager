...
│   ├── pages/
│   │   ├── operations/                    # Operations log list (paginated, searchable); verified users can use FAB → blank new form, while unverified users see the same history in read-only mode; first open shows loading until the initial request settles; rapid page returns reuse a short freshness window instead of reloading on every onShow
│   │   ├── inventory/                     # Current inventory list; verified users can tap item → action sheet → pre-filled locked form and use FAB → pre-filled 入库 form; unverified users see the same inventory in read-only mode; first open shows loading until the initial request settles; rapid page returns reuse a short freshness window instead of reloading on every onShow
...
│   │   ├── settings/                      # Role-aware settings: display name + role badge; unverified users also see an inline notice that warehouse operation access must be requested from the department lead; org management (admin+); user approval/management (admin+); admin promote/demote + chairman transfer (chairman only); Excel export (verified) + xlsx import (chairman)
...
`operationsList` and `inventoryList` no longer issue `count(*)` for every page. They fetch `pageSize + 1` rows and return `hasMore`, which matches the miniprogram's infinite-scroll UX and removes exact-count overhead from hot list endpoints. Both list endpoints are readable by any logged-in user, including `unverified`, while write paths remain restricted to verified roles.
